...
HP_SPECIAL_MODEL = os.getenv('HP_SPECIAL_MODEL', os.path.join(BASE_DIR, 'data', 'HP_special', 'model.txt'))
...
def UploadLabel():
    try:
        ...
        for i, file_info in enumerate(selected_files):
            ...
            sanitized_file_name = unique_file_name.replace("/", "_")

            # Save the file to the data folder
            file_path = os.path.join(DATA_FOLDER, sanitized_file_name)

            print(f"Saving file: {sanitized_file_name}")
...
def draw_text(draw, x, y, line, font, line_spacing):
...
def generate_image():
    ...
    base_name = re.sub(r'(_\d+)?\.txt$', '', selected_text_files)
    ...
    for idx, file_name in enumerate(related_files, start=1):
        ...
        for line in lines:
            ...
            if match and match.group(1) == "bold":
                font = bold_font
            ...
            y = draw_text(draw, x, y, line, font, line_spacing)
    ...
    if special_HP_flag:
        ...
        with open(HP_Template, 'r') as file:
            ...
            for HP_line in lines:
                ...
                y = draw_text(draw_special, x, y, HP_line, font, line_spacing)
